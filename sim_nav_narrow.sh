...
LOG_DIR="${AUTO_EVAL_LOG_DIR:-${SCRIPT_DIR}/.auto_eval_logs/sim_nav_narrow}"
...
if [[ "${SKIP_RVIZ}" != "1" ]]; then
  tabs+=("RVIZ|ros2 run rviz2 rviz2 -d ${SCRIPT_DIR}/rm_navi/rm_navigation/navi/rviz/nav2_default_view.rviz")
fi
...
if [[ "${HEADLESS_MODE}" = "1" ]]; then
  echo "Headless navigation stack launched."
  exit 0
fi
