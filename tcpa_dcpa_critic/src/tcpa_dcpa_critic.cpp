...
double TCPADCPACritic::scoreTrajectory(const dwb_msgs::msg::Trajectory2D & traj)
{
  ...
  const double robot_px = traj.poses.front().x;
  const double robot_py = traj.poses.front().y;
  const double sigma_safe_sq = sigma_safe_ * sigma_safe_;
  double total_cost = 0.0;

  for (const auto & obstacle : obstacles) {
    const double rel_px = obstacle.position.x - robot_px;
    const double rel_py = obstacle.position.y - robot_py;
