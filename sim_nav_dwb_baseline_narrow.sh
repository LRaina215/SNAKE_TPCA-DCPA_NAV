...
LOG_DIR="${AUTO_EVAL_LOG_DIR:-${SCRIPT_DIR}/.auto_eval_logs/sim_nav_dwb_baseline_narrow}"
...
if [[ "${HEADLESS_MODE}" = "1" ]]; then
  echo "Headless narrow-corridor DWB baseline navigation stack launched."
  exit 0
fi

echo "Opened narrow-corridor DWB baseline navigation tabs."
