...
WORLD_FILE="${SCRIPT_DIR}/tcpa_sim_env/worlds/narrow_corridor.world"
...
if [[ ! -f "${WORLD_FILE}" ]]; then
  echo "Missing world file: ${WORLD_FILE}"
  exit 1
fi
...
LOG_DIR="${AUTO_EVAL_LOG_DIR:-${SCRIPT_DIR}/.auto_eval_logs/sim_pre_narrow}"

tabs=(
  "Sim|ros2 launch tcpa_sim_env sim_launch.py gui:=$([[ \"${HEADLESS_MODE}\" = \"1\" ]] && echo false || echo true) world:=${WORLD_FILE} scene_mode:=narrow_corridor"
...
if [[ "${HEADLESS_MODE}" = "1" ]]; then
  echo "Headless narrow-corridor pre-simulation stack launched."
  exit 0
fi

echo "Opened narrow-corridor pre-simulation tabs."
