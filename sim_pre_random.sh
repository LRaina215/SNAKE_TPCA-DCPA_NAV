...
WORLD_FILE="${SCRIPT_DIR}/tcpa_sim_env/worlds/random_crowd.world"
...
LOG_DIR="${AUTO_EVAL_LOG_DIR:-${SCRIPT_DIR}/.auto_eval_logs/sim_pre_random}"

tabs=(
  "Sim|ros2 launch tcpa_sim_env sim_launch.py gui:=$([[ \"${HEADLESS_MODE}\" = \"1\" ]] && echo false || echo true) world:=${WORLD_FILE} scene_mode:=random_crowd"
...
if [[ "${HEADLESS_MODE}" = "1" ]]; then
  echo "Headless random-crowd pre-simulation stack launched."
  exit 0
fi

echo "Opened random-crowd pre-simulation tabs."
